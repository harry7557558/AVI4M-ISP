...
#include "../.glsl.cpp"
#else
	const vec3 BOX_RADIUS = vec3(2.0f);
...
int main(int argc, char* argv[]) {
	...
	vec3 cell_size = 2.0f*BOX_RADIUS / (vec3(box0) * exp2(boxe));
	...
	// buffer
	std::vector<uint16_t> buffer = ScalarFieldTriangulator_octatree::octatree_buffer(
		[&](vec3 p) { eval_count++; return GLSL::map(p, false).w; },
		-BOX_RADIUS, BOX_RADIUS,
		box0, boxe, 8
	);
	printf("Buffer: %.2f MB\n", buffer.size() * 2 / exp2(20));
