...
#define EDGE_PRECISION 128
#define MESH_SIZE (GRID_SIZE*EDGE_PRECISION)
	...
	std::vector<uint16_t> treeSampler;
	uint getUint16(uint i) {
		return treeSampler[i];
	}
	uint getUint32(uint i) {
		if (i >= (int)treeSampler.size()) {
			printf("%d %d\n", i, (int)treeSampler.size());
			return 0;
		}
		return treeSampler[i] + treeSampler[i + 1] * 0x10000;
	}
	vec3 getVec3(uint i) {
		uint x = getUint16(i);
		uint y = getUint16(i + 1);
		uint z = getUint16(i + 2);
		return mix(P0, P1, vec3(x, y, z) / vec3(MESH_SIZE));
	}
	...
	struct TreeNode {
		...
		uint ptr;  // position in the buffer
	};
	...
	bool intersectObject(vec3 ro, vec3 rd, float &min_t, float t1, vec3 &min_n, vec3 &col) {
		...
		for (int xi = 0; xi < SEARCH_DIF.x; xi++) for (int yi = 0; yi < SEARCH_DIF.y; yi++) for (int zi = 0; zi < SEARCH_DIF.z; zi++) {
			int grid_pos = getUint32(2 * ((zi * SEARCH_DIF.y + yi) * SEARCH_DIF.x + xi));
			...
			while (true) {
				...
				if (cur.ptr != 0) {
					// triangles
					if (cell_size == 1) {
						uint n = getUint16(cur.ptr);
						for (int ti = 0; ti < n; ti++) {
							vec3 a = getVec3(cur.ptr + 11 * ti + 1);
							vec3 b = getVec3(cur.ptr + 11 * ti + 4);
							vec3 c = getVec3(cur.ptr + 11 * ti + 7);
							t = intersectTriangle(ro, rd, a, b, c);
							if (t > 0.0 && t < min_t) {
								min_t = t, min_n = cross(b - a, c - a);
								uint rg = getUint16(cur.ptr + 11 * ti + 10);
								uint b = getUint16(cur.ptr + 11 * ti + 11);
								col = vec3(rg % 256, rg / 256, b) / 255.0;
							}
						}
						cur.ptr = 0;
					}
				...
				// next node
					...
					if (cur.subcell >= 8) {
						...
						cur.ptr = getUint32(cur.ptr + 2 * cur.subcell);
						cur.subcell = 0;
					}
				}
	...
	vec3 mainRender(vec3 ro, vec3 rd) {
		...
		vec3 m_col = vec3(1.0f), t_col = vec3(0.0f), col;
		...
		for (int iter = 0; iter < 64; iter++) {
			...
			if (intersectObject(ro, rd, t, min_t, n, col) && t < min_t) {
				...
				material = MAT_OBJECT;
				//return vec3(1.0) * max(dot(min_n, -rd), 0.0);
			}
			//return vec3(0, 0, 0);
			...
			min_n = dot(rd, min_n) < 0.f ? min_n : -min_n;  // ray hits into the surface
			ro += rd * min_t;
...
void exportTree() {
	GLSL::treeSampler = ScalarFieldTriangulator_octatree::octatree_buffer(
		GLSL::sdf,
		P0, P1,
		SEARCH_DIF, PLOT_DEPTH, EDGE_PRECISION,
		GLSL::colorf
	);
	printf("Tree sampler: %.2f MB\n", GLSL::treeSampler.size() * 2 / exp2(20));

#if 1
	FILE* fp = fopen("D:\\.bin", "wb");
	fwrite(&GLSL::treeSampler[0], 2, GLSL::treeSampler.size(), fp);
	fclose(fp);
#else
	...
	fread(&GLSL::treeSampler[0], 2, size, fp);
	fclose(fp);
#endif
}
