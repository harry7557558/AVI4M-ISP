	...
	std::vector<uint16_t> to_buffer() {
		...
		for (int h = PLOT_DEPTH; h--;) {
			...
			for (Pi pc : pc_map) {
				...
				int i = pc.i;
				if (top_layer.empty() || top_layer.back().p != p) {
					Node n;
					n.p = p;
					for (int _ = 0; _ < 8; _++) if (p->c[_] == layer[i].p) n.c[_] = i;
					top_layer.push_back(n);
				}
				else {
					Node n = top_layer.back();
					for (int _ = 0; _ < 8; _++) if (p->c[_] == layer[i].p) n.c[_] = i;
					top_layer.back() = n;
				}
			}
			middle_layers.push_back(top_layer);
			layer = top_layer;
		}
		...
		for (int z = 0; z < SEARCH_DIF.z; z++) {
			for (int y = 0; y < SEARCH_DIF.y; y++) {
				for (int x = 0; x < SEARCH_DIF.x; x++) {  // O(N²) should be ok
					int j = (z * SEARCH_DIF.y + y) * SEARCH_DIF.x + x;
					octatree_node *p = &octatree_grid[x][y][z];
					for (int i = 0; i < (int)middle_layers.back().size(); i++) {
						if (middle_layers.back()[i].p == p) {
							top_layer[j] = i;
							break;
						}
					}
		...
		int add = 2 * (int)top_layer.size();  // top layer
		for (int i = 0; i < (int)top_layer.size(); i++) {
			int data = top_layer[i];
			int p = data == -1 ? 0 : data + add;
		...
		for (int i = 0; i < (int)bottom_layer.size(); i++)  // bottom layer
			add += 1 + 2 * 3 * bottom_layer[i].n;
		for (int i = 0; i < (int)bottom_layer.size(); i++) {
			...
			for (int j = 0; j < bottom_layer[i].n; j++) {
				...
				for (int _ = 0; _ < 3; _++) {
					int data = ((int*)&triangles_v[trig_i])[_];
					if (data < 0 || data >= (int)vertices.size())
		...
		add += 3 * (int)vertices.size();  // vertices
		for (int i = 0; i < (int)vertices.size(); i++) {
			result.push_back((uint16_t)vertices[i].x);
			result.push_back((uint16_t)vertices[i].y);
			result.push_back((uint16_t)vertices[i].z);
		}
		return result;
	}
