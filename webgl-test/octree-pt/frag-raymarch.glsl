...
vec4 mapContent(vec3 p, bool col_required) {  // demo
    float d = sdLnNormEllipsoid(p-vec3(0,0,-0.0), vec3(1.2,1.2,2.4), vec3(1.2));
    return vec4(1.0,0.5+0.1*p.z-0.05*p.y,0.0, d);
}
...
vec3 mainRender(vec3 ro, vec3 rd) {
    ...
    int loopCount = 0;
    int glassIntersectCount = 0;
    int contentIntersectCount = 0;
    ...
    //return vec3(loopCount, glassIntersectCount, contentIntersectCount) * 10.0/255.0;
    return m_col + t_col;
}
